...
export default class MyBartersScreen extends Component {
  static navigationOptions = { header: null };
   ...
   sendNotification=(itemDetails, requestStaus)=>{
     ...
     .then((snapshot)=>{
       snapshot.forEach((doc)=>{
         var message=''
         if(requestStaus==="ItemSent"){
           message=this.state.donorName+"sent you the item"
         }
         else{
           message=this.state.donorName="has shown intrest in donating the intrest"
         }
   ...
   sendItem=(itemDetails)=>{
    ...
    if(itemDetails.requestStatus==="Item sent"){
      ...
      console.log('asdf', itemDetails.doc_id)
      db.collection("exchange_requests").doc(itemDetails.doc_id).update({
        "request_status": "donor intrested12"
      })
      this.sendNotification(itemDetails, requestStatus)
    }else{
      var requestStatus="Item Sent"
      db.collection("exchange_requets")
